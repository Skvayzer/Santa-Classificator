...
MODEL_LOAD_PATH = "../input/modelsanta/resnet152.pt"
...
exp_lr_scheduler_pretrain = lr_scheduler.StepLR(pretrain_optimizer, step_size=50, gamma=0.1)

# Pretrain
# запустить предобучение модели на две эпохи
pretrain_results = train_model(model, loaders, criterion, pretrain_optimizer, exp_lr_scheduler_pretrain,
            phases=['train', 'val'], num_epochs=30)


model.load_state_dict(best_model_wts)
torch.save(model.state_dict(), MODEL_WEIGHTS)

train_optimizer = torch.optim.SGD(model.parameters(), lr=0.0001, momentum=0.9)


# Decay LR by a factor of 0.1 every 7 epochs
exp_lr_scheduler_train = lr_scheduler.StepLR(train_optimizer, step_size=50, gamma=0.1)
...
train_results = train_model(model, loaders, criterion, train_optimizer, exp_lr_scheduler_train,
            phases=['train', 'val'], num_epochs=500)
